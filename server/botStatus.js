  ...
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(logFilePath, 'utf8', (err, data) => {
      if (err) {
        ...
        // Фильтруем строки, которые содержат предупреждения MongoDB и ошибки Polling
        const warningLogs = data.split('\n').filter((line) => 
          line.includes('[MONGODB DRIVER] Warning') || // Предупреждения MongoDB
          line.includes('Polling error') // Ошибки Polling
        );

        const warningCount = warningLogs.length; // Подсчитываем количество предупреждений и ошибок
        ...
        // Возвращаем JSON объект с логами и количеством предупреждений/ошибок
        resolve({
          logs: formattedLogs, // Логи с экранированными переносами строк
          warningCount, // Количество предупреждений и ошибок
          message: 'Logs and warning count (including errors) fetched successfully',
        });
      }
    });
  });
